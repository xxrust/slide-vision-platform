...
**平台层** (`src/`):
- `Slide.Platform.Abstractions` (netstandard2.0): 统一算法接口层
  - 插件接口: `IAlgorithmPlugin`, `IAlgorithmSession`, `SimpleAlgorithmInput`, `SimpleAlgorithmResult`
  - 引擎接口: `IAlgorithmEngine`, `AlgorithmInput`, `AlgorithmResult`, `AlgorithmEngineIds`, `AlgorithmEngineDescriptor`, `AlgorithmExecutionStatus`, `AlgorithmMeasurement`
  - Schema 自描述: `IAlgorithmEngineMetadata`, `AlgorithmParameterSchema`, `AlgorithmOutputSchema`
- `Slide.Platform.Runtime` (netstandard2.0): 插件运行时
  - `PluginLoader` — DLL 动态加载
  - `AlgorithmRegistry` — 插件注册表
  - `EngineRegistry` — 实例级引擎注册表
  - `PluginEngineAdapter` — 将 `IAlgorithmPlugin` 桥接为 `IAlgorithmEngine`
...
| 文件 | 职责 |
|------|------|
| `Page1.xaml.cs` | 主 UI 逻辑、检测流程编排、VM 回调处理、图像管线（`BuildAlgorithmInput`、`SelectImageFilesAsync` 等） |
...
**统一接口** 定义在 `Slide.Platform.Abstractions`:
- `IAlgorithmEngine` — 引擎接口（`ExecuteAsync(AlgorithmInput, CancellationToken)`）
- `AlgorithmInput` — 丰富输入（模板名、图像路径字典、参数字典等）
- `AlgorithmResult` — 丰富结果（状态、测量数据、渲染图像、调试信息等）
- `IAlgorithmEngineMetadata` — 可选的参数/输出 Schema 自描述接口

**引擎实现** 位于 `PlatformHost.Wpf/Algorithms/`:
- 引擎 ID 常量: `AlgorithmEngineIds.OpenCv`, `.Onnx`, `.OpenCvOnnx`, `.Vm`
- `AlgorithmEngineRegistry` — 静态引擎注册表，支持 `Register()`/`Unregister()` 动态注册
- `OpenCvAlgorithmEngine` — OpenCV 引擎（实现 `IAlgorithmEngineMetadata`）
- `OnnxAlgorithmEngine` — ONNX 引擎
- `OpenCvOnnxAlgorithmEngine` — 组合引擎（并行执行 OpenCV + ONNX）
- `Slide.Algorithm.Contracts/` — 已清空的空壳项目（类型已迁移到 Abstractions）
...
- 外部插件可通过 `PluginEngineAdapter` 桥接为 `IAlgorithmEngine`，再通过 `AlgorithmEngineRegistry.Register()` 动态注册
- 引擎可实现 `IAlgorithmEngineMetadata` 自描述参数和输出 Schema
