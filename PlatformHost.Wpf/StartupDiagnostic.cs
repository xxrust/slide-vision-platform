...
namespace WpfApp2
{
    ...
    public static class StartupDiagnostic
    {
        ...
        private static void CheckCriticalDlls()
        {
            ...
            bool shouldCheck3D = File.Exists(Path.Combine(exeDir, "GlueInspect.ThreeD.Host.exe"))
            ...
            string disable3D = Environment.GetEnvironmentVariable("GLUEINSPECT_DISABLE_3D");
            if (!string.IsNullOrWhiteSpace(disable3D))
            {
                LogInfo($"GLUEINSPECT_DISABLE_3D={disable3D} (main process shields 3D)");
            }
